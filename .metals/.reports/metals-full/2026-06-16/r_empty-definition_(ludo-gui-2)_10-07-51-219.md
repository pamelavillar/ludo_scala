error id: file://<HOME>/Downloads/ludo-gui%202/src/main/scala/ludo/gui/components/BoardView.scala:
...
class BoardView(board: InterfazTablero, onPieceClick: Int => Unit) extends GridPane:
  ...
  // colorOpcional = Some(color) para casillas de color de entrada/llegada
   ...
   val trackposicions: Vector[(Int, Int)] = (
      ...
      (1 to 5).map(c =>(c,6)) ++ //fila ploma superior de rojo
      (0 to 5).map(r =>(6,r)).reverse ++ //fila ploma izquierda zona verde
      (7,0) ++ //cuadrado de medio - camino casa verde
